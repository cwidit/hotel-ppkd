...
<div class="page-title mb-3">
    <div class="row align-items-center">
        <div class="col-md-8">
            ...
            <p class="text-subtitle text-muted mb-0">{{ now('Asia/Jakarta')->translatedFormat('l, d F Y') }}</p>
...
<section class="section">
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        <div class="stat-value text-primary">{{ $vacantRooms }} <small style="font-size:.9rem;color:#aaa;">/ {{ $totalRooms }}</small></div>
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        <div class="stat-value" style="font-size:1.2rem;color:#333;">Rp {{ number_format($todayRevenue, 0, ',', '.') }}</div>
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Kalender Ketersediaan Kamar (14 Hari ke Depan)</h4>
                    <div>
                ...
                <div class="card-body p-0">
                    <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-bordered table-sm m-0" style="font-size: 11px; white-space: nowrap;">
                            <thead class="thead-light sticky-top">
                                <tr>
                                    <th style="min-width: 80px;">Kamar</th>
                                    @foreach($dateRange as $date)
                                        <th class="text-center {{ $date->isWeekend() ? 'text-danger' : '' }}">{{ $date->format('d/m') }}</th>
                                    @endforeach
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr>
                                    <td class="font-weight-bold">{{ $room->room_number }} <span class="text-muted d-block" style="font-size:9px;">{{ $room->roomType->name }}</span></td>
                                    @foreach($dateRange as $date)
                                        @php
                                            $res = $calendarData[$room->id][$date->format('Y-m-d')];
                                        @endphp
                                        @if($res)
                                            <td class="bg-primary text-white text-center p-0 align-middle" title="{{ $res->guest->first_name }} ({{ $res->booking_number }})" style="cursor:pointer;" onclick="window.location='{{ route('admin.reservations.show', $res->id) }}'">
                                                <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                                                    <i data-feather="user" style="width:12px;height:12px;"></i>
                                                </div>
                                            </td>
                                        @else
                                            <td class="text-center align-middle" style="background-color: #f8f9fc;">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                <tr style="cursor:pointer;" onclick="window.location='{{ route('admin.reservations.show', $res->id) }}'">
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span class="badge badge-{{ $sc }}">{{ str_replace('_', ' ', $res->status) }}</span>
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        @endphp
                                        <span class="badge badge-{{ $pc }}">{{ str_replace('_', ' ', $res->payment_status) }}</span>
                                    </td>
                                </tr>
                                @empty
                                <tr><td colspan="5" class="text-center text-muted py-3">Belum ada reservasi.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="col-md-4">
            ...
            <div class="card mb-4">
                <div class="card-header"><h4 class="mb-0">Status Kamar</h4></div>
                <div class="card-body">
                    <canvas id="roomStatusChart" height="180"></canvas>
                    <div class="mt-3">
                        <div class="d-flex justify-content-between mb-1">
                            <span><span style="display:inline-block;width:10px;height:10px;background:#28a745;border-radius:50%;margin-right:5px;"></span>Vacant</span>
                            <strong>{{ $roomStatuses['Vacant'] }}</strong>
                        </div>
                        <div class="d-flex justify-content-between mb-1">
                            <span><span style="display:inline-block;width:10px;height:10px;background:#1d7af3;border-radius:50%;margin-right:5px;"></span>Occupied</span>
                            <strong>{{ $roomStatuses['Occupied'] }}</strong>
                        </div>
                        <div class="d-flex justify-content-between">
                            <span><span style="display:inline-block;width:10px;height:10px;background:#dc3545;border-radius:50%;margin-right:5px;"></span>Maintenance</span>
            ...
            {{-- Quick Actions --}}
            <div class="card">
                <div class="card-header"><h4 class="mb-0">Akses Cepat</h4></div>
                <div class="card-body p-2">
                    <a href="{{ route('admin.calendar.index') }}" class="btn btn-outline-primary btn-block mb-2 text-left">
                        <i data-feather="grid" style="width:14px;height:14px;margin-right:6px;"></i> Kalender Okupansi
                    </a>
                    <a href="{{ route('admin.housekeeping.index') }}" class="btn btn-outline-warning btn-block mb-2 text-left">
                        <i data-feather="layers" style="width:14px;height:14px;margin-right:6px;"></i> Housekeeping Board
                    </a>
                    <a href="{{ route('admin.reports.daily') }}" class="btn btn-outline-info btn-block mb-2 text-left">
